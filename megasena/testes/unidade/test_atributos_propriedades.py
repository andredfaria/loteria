...
from megasena.infra.atributos.builder import FeatureBuilder
...
def test_coocorrencia_lift_near_one_under_independence(draws):
    ...
    Megasena is a sparse lottery (esperado(n, m) << 1 in a 30-draw window).
    Measured on this fixture's seed: no-evidence-as-0.0 without smoothing
    gave |dev|~0.17; either fix alone narrows it; both together (current
    code) gives a measured grand mean of ~0.954 (|dev|~0.046). The residual
    bias is Jensen's-inequality shrinkage, documented in the docstring, not
    a bug -- but the tolerance below is tight enough that regressing either
    fix (or losing the fix in a refactor) makes this test fail.
    """
    means = []
