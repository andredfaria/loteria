...
from megasena.dominio.regras import NUMEROS_POR_SORTEIO, TOTAL_NUMEROS
...
def load_draws(dados_dir: Path) -> List[Sorteio]:
    sorteios = []
    for path in sorted(dados_dir.glob("megasena_*.json")):
        try:
...
def _parse_raw(raw: dict) -> Sorteio | None:
    try:
        ...
        if not all(1 <= d <= TOTAL_NUMEROS for d in dezenas):
            ...
            return None
        return Sorteio(concurso=concurso, data=data, dezenas=sorted(dezenas))
