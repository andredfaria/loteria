...
from megasena.infra.config import (
    API_MEGASENA,
    API_RETRIES,
    API_RETRY_MAX,
    API_RETRY_MIN,
...
class MegasenaFetcher:
    ...
    def _load_local_files(self) -> List[dict]:
        records = []
        files = sorted(self.data_dir.glob("megasena_*.json"))
    ...
    def _fetch_concurso_api(self, numero: int) -> Optional[dict]:
        try:
            raw = self._get_api(f"{API_MEGASENA}/{numero}")
    ...
    def _fetch_latest_api(self) -> Optional[dict]:
        try:
            raw = self._get_api(f"{API_MEGASENA}/latest")
            return _parse_record(raw)
        except Exception as exc:
    ...
    def _fetch_all_api(self) -> List[dict]:
        try:
            raw_list = self._get_api(API_MEGASENA)
            records = []
            for raw in raw_list:
                rec = _parse_record(raw)
                if rec:
    ...
    def _save_concurso_json(self, concurso: int, raw: dict) -> None:
        path = self.data_dir / f"megasena_{concurso}.json"
        if not path.exists():
