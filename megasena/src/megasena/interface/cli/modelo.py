...
from megasena.infra.config import DADOS_DIR, MODELOS_DIR, NUMEROS_POR_SORTEIO, TOTAL_NUMEROS
from megasena.infra.dados.leitor import load_draws
from megasena.infra.modelos.ensemble import EnsemblePredictor
from megasena.infra.geracao.optimizers import gerar_jogo
...
def prever() -> None:
    ...
    filtros_nome = {
        "pares": "Paridade (2-4 pares)",
        "baixos": "Faixa (1-5 baixos)",
        "soma": "Soma (120-240)",
        "consecutivos": "Consecutivos (≥1 par)",
        "repeticoes": "Repetições (≤2 do anterior)",
    }
...
def probas() -> None:
    ...
    from megasena.infra.atributos.base import atraso
...
def backtest() -> None:
    ...
    console.print(f"[cyan]Backtest walk-forward até 20 testes...[/cyan]")

    with console.status("[cyan]Executando backtest (pode levar vários minutos)...[/cyan]"):
        for i in range(len(draws) - n_testes, len(draws) - 1, step):
            ...
            pred.fit(train)
            p = pred.score()
            top6 = sorted(np.argsort(p)[::-1][:6])
            top6 = [int(x + 1) for x in top6]
            hits_ml.append(len(set(top6) & set(test.dezenas)))
