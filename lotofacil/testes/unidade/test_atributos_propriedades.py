...
from lotofacil.infra.atributos.advanced import (
    coocorrencia_score,
    trend_score,
    volatilidade_score,
)
...
def _with_mutated_draw(draws: List[Draw], pos: int) -> List[Draw]:
    ...
    # Lotofacil draws 15 of 25 numbers, so the complement pool has only 10
    # candidates -- not enough to build a full replacement draw. Rotate a
    # couple of numbers instead of fully complementing.
    new_dezenas = sorted(
        (set(original.dezenas) - set(original.dezenas[:2])) | set(replacement_pool[:2])
    )
    mutated[pos] = Draw(
...
def test_coocorrencia_lift_near_one_under_independence(draws):
    ...
    averaged across numbers and idx values must land close to 1.0.

    Lotofacil draws 15 of 25 numbers, so esperado(n, m) is comfortably above
    5 with the default k=30/top=10 -- this was already the "OK" case in the
    review (both fixes barely move the measured mean, ~0.979 either way),
    and the tolerance here is tight to confirm it stays that way.
    """
    means = []
    ...
    assert abs(grand_mean - 1.0) < 0.05, f"grand mean lift={grand_mean}, too far from 1.0"
