"""Validate neural model performance on historical draws."""
...
def validate_neural(draws: list[Draw], test_window: int = 50, train_window: int = 300,
                     filtered: bool = False) -> dict:
    """
    Walk-forward validation of the neural model.

    For each draw in the test window:
    - Train on previous `train_window` draws
    - Predict the next draw
    - Compare prediction with actual result

    Args:
        draws: all historical draws
        test_window: number of draws to test on (from the end)
        train_window: number of draws to use for training
    """
    ...
    console.print(f"[bold]Validating neural model on {test_window} draws...[/bold]")
    ...
    for i in range(test_window):
        ...
        try:
            ...
            if filtered:
                top11 = neural.predict_with_filters(train_draws)
            else:
                probas = neural.predict_proba()
                top11 = sorted(np.argsort(probas)[::-1][:11] + 1)

            hits = contar_acertos(top11, actual_draw.dezenas)
...
def validate_neural_loaded(draws: list[Draw], test_window: int = 50, filtered: bool = False) -> dict:
    """
    Validate using a pre-trained neural model (no retraining).

    Uses the saved model to predict the last `test_window` draws.
    This is faster but less realistic (model sees all history).
    """
    from strategies.eleven_numbers.approaches.neural import NeuralApproach
    ...
    console.print(f"[bold]Validating loaded neural model on last {test_window} draws...[/bold]")
...
def print_results(result: dict):
    ...
    table.add_row("13+ Hits", f"{result['hit_13_plus']}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Validate neural model")
    ...
    if args.loaded:
        result = validate_neural_loaded(draws, test_window=args.window, filtered=args.filtered)
    else:
        result = validate_neural(draws, test_window=args.window, train_window=args.train,
