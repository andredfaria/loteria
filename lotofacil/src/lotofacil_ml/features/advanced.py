...
from lotofacil_ml.features.base import freq_k, atraso as calc_atraso
...
def coocorrencia_score(draws: List[Draw], idx: int, k: int = 30) -> Dict[int, float]:
    """
    For each number N, count how many times N appeared alongside other numbers
    in the k draws before idx. Normalized by window size.
    """
    window = draws[max(0, idx - k):idx]
    score = {n: 0.0 for n in _NUMBERS}
    for d in window:
        dez_set = set(d.dezenas)
        co_count = len(dez_set) - 1  # each draw contributes 14 co-occurrences per number
        for n in dez_set:
            score[n] += co_count
    total = len(window)
    if total > 0:
