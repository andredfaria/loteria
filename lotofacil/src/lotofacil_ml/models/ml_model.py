"""Ensemble ML model: RF + XGBoost + LightGBM via MultiOutputClassifier."""
...
from lotofacil_ml.config import (
    RANDOM_SEED, RF_MAX_DEPTH,
    RF_MIN_SAMPLES_LEAF, RF_N_ESTIMATORS, TOTAL_NUMBERS,
)
...
class MLEnsembleModel(BaseModel):
    """Multi-output soft-voting ensemble for number probability estimation."""
    ...
    def predict_proba(self) -> np.ndarray:
        ...
        # probas: list of (1, n_classes) arrays — take P(class=1).
        # When only one class was seen during training, sklearn returns a single
        # column; in that case infer from the class label stored in estimators_.
        result = np.empty(TOTAL_NUMBERS, dtype=np.float32)
        for j, (p, est) in enumerate(zip(probas, self._model.estimators_)):
            if p.shape[1] == 2:
                result[j] = p[0, 1]
            else:
                # Single class — check which class the estimator knows
