...
para um teste unitário, então os testes que exercitam o CLI usam um duplo de
teste (monkeypatch) para `fit`/`save`/`load`/`score`/`score_dict` do
`EnsemblePredictor`, evitando o treino real.
"""
...
from diadesorte.infra.config import NUMEROS_POR_SORTEIO, TODOS_MESES, TOTAL_NUMEROS
from diadesorte.infra.modelos.ensemble import EnsemblePredictor
from diadesorte.infra.modelos.frequency_ensemble import FrequencyEnsembleModel
from diadesorte.interface.cli import modelo as modelo_cli
...
def _draws(n: int = 60) -> list[Sorteio]:
    ...
    return [
        Sorteio(
            ...
            mes_sorte=rng.choice(TODOS_MESES),
        )
        for i in range(n)
    ]
