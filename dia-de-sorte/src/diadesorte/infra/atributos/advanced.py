...
from diadesorte.infra.atributos.base import NUMEROS, freq_k
...
def coocorrencia_score(
    ...
    (k=30, top=10, 8680 amostras, seed=99), a media medida foi 0.931 (desvio
    de 0.069 abaixo de 1.0), nao exatamente 1.0. O vies vem de desigualdade
    de Jensen -- E[obs/esp] < E[obs]/E[esp] quando `esp` e pequeno, como e o
    caso aqui (esperado(n,m) tipicamente pouco acima de 1 numa janela de 30
    concursos nesta loteria). Nao trate 0.931 como "quase certo" nem invente
    uma correcao de vies sem reportar -- o numero fica ai porque foi medido,
    nao porque e o alvo.
    ...
    unidades. A suavizacao com `alpha` (padrao 1.0) reduz esse viés (0.106
    sem suavizacao -> 0.069 com suavizacao, no mesmo experimento), mas nao o
    elimina; ainda assim, com `esperado` muito baixo, nao espere um sinal
    fino -- espere algo perto de 1.0 quase sempre, com saltos discretos
    ocasionais. Um WARNING e emitido quando o esperado medio da janela fica
    abaixo de um limiar de confianca (ver `_ESPERADO_MINIMO_CONFIAVEL`); nao
    interprete valores altos como evidencia de "pares que saem juntos" mesmo
    quando o warning nao dispara.
    """
