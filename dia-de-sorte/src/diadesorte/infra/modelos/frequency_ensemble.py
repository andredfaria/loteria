...
from diadesorte.infra.modelos.base_model import BaseModel
...
class FrequencyEnsembleModel(BaseModel):
    ...
    def fit(self, draws: List[Draw]) -> None:
        if not draws:
            logger.warning("FrequencyEnsembleModel.fit() called with empty draws")
            return
        n = len(draws)
        binary = np.zeros((n, TOTAL_NUMEROS), dtype=np.float32)
        for i, d in enumerate(draws):
            for num in d.dezenas:
                binary[i, num - 1] = 1.0

        combined = np.zeros(TOTAL_NUMEROS, dtype=np.float32)
        total_weight = 0.0
        ...
        if hi > lo:
            ...
            self._scores = np.ones(TOTAL_NUMEROS, dtype=np.float32) * 0.5
