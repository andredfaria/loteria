...
def simulated_annealing(
    ...
    for _ in range(n_restarts):
        current = sorted(random.sample(pool, 7))
...
def selecionar_mes(draws: List[Draw]) -> str:
    freq_mes = {m: 0 for m in TODOS_MESES}
    for d in draws:
        if d.mes_sorte in freq_mes:
            freq_mes[d.mes_sorte] += 1
...
def gerar_jogo(
    ...
    mes_sorte = selecionar_mes(draws)
